...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && DISCONNECT_VPN.equals(intent.getAction())) {
            if (mManagement != null)
                mCommandHandler.post(() -> mManagement.stopVPN(false));
            return START_NOT_STICKY;
        }
    ...
    private void startOpenVPN(Intent intent, int startId) {
        ...
        if (!useOpenVPN3) {
            try {
                String configToWrite = mProfile.getConfigFile(this, false);
                android.util.Log.d("CorvusDiagnostics", "Config being written to OpenVPN (length=" + configToWrite.length() + "):");
                // Log only CA block for safety
                int caStart = configToWrite.indexOf("<ca>");
                int caEnd = configToWrite.indexOf("</ca>");
                if (caStart != -1 && caEnd != -1) {
                    android.util.Log.d("CorvusDiagnostics", "CA BLOCK IN CONFIG: " + configToWrite.substring(caStart, caEnd + 5));
                }
    ...
    private ParcelFileDescriptor openTun(TunConfig tc) {
        ...
        VpnStatus.logInfo(R.string.last_Corvus_VPN_tun_config);
